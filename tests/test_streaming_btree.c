...
void check_leaf_node(node_header * leaf)
{
	...
	for(i=1;i<leaf->numberOfEntriesInNode;i++)
	{
		...
		if(ret > 0)
		{
			DPRINT("FATAL corrupted leaf index at index %llu total entries %llu\n", (LLU)i, (LLU)leaf->numberOfEntriesInNode);
			DPRINT("previous key is: %s\n", (char *)prev+sizeof(int32_t));
			DPRINT("curr key is: %s\n", (char *)curr+sizeof(int32_t));
			raise(SIGINT);
			exit(-1);
		}
    DPRINT("previous key is: %d : %s\n", *(uint32_t *)prev, (char *)prev+sizeof(int32_t));
		DPRINT("curr key is: %d : %s\n", *(uint32_t *)curr, (char *)curr+sizeof(int32_t));
	}
}
...
int main()
{
  ...
  uint64_t size = 100 * 1024 * 1024 * 1024;
  uint64_t i;
  int32_t j;
  ...
  /*open db*/
  db_handle * handle = db_open(VOLUME_NAME, 0, size, DB_NAME, O_CREATE_DB);
  region.db = handle;
  ...
  for(i=0;i<NUM_OF_KEYS;i++){
    ...
    *(uint32_t *)(key_buf+sizeof(uint32_t)+*(uint32_t *)key_buf) = 900;
    ...
    if(i%100000 == 0){
      DPRINT("added key %llu size is %d %s\n",(LLU)i,*(uint32_t *)key_buf, key_buf+sizeof(uint32_t));
    }
  }
  ...
  for(i=0;i<NUM_OF_KEYS;i++){
    ...
    *(uint32_t *)key_buf = strlen(key_buf+4);
    if(i%100000 == 0){
      DPRINT("read key %llu size is %d %s\n",(LLU)i,*(uint32_t *)key_buf, key_buf+sizeof(uint32_t));
    }
    assert(_lookup_key(handle, key_buf) != NULL);
  }
  DPRINT("Test passed successfully! :-)\n");
  return SUCCESS;
}
