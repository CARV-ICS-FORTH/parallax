...
struct tu_data_message *Client_Receive_N_Messages_Semaphore_Blocking_NotReceiving( client_region* cli_tu_region, struct tu_data_message *data_message , int next_mail );
static void *client_thread_receiving_messages_RDMA(void *args);

#if TU_SEMAPHORE
static struct tu_data_message *client_blocking_receive( client_region* cli_tu_region, struct tu_data_message *data_message , int next_mail );
#endif
	...
	for ( i = 0; i < client_regions->num_regions; i ++ ){
		if(client_regions->tu_regions[i]->head_net != NULL){
			if ( ( client_regions->tu_regions[i]->head_net->num_NICs > 0 ) 
				...
				client_regions->tu_regions[i]->rdma_conn[0] = 
				crdma_client_create_connection_list_hosts((void*)client_regions->channel, client_regions->tu_regions[i]->head_net->IPs,
				...
				client_regions->tu_regions[i]->ID_region.minimum_range,client_regions->tu_regions[i]->head_net->hostname,NUM_OF_CONNECTIONS_PER_SERVER);
				...
				client_regions->tu_regions[i]->ID_region.minimum_range,client_regions->tu_regions[i]->head_net->hostname);
			}
		}
	}
}
...
void get_server_rdma_conn(client_region * region, void *channel)
{
  ...
  if(region->head_net->number_of_mapped_regions == 0){ 
      ...
      DPRINT("Creating number (%d) of a total of %d connection(s) with server %s\n",i, NUM_OF_CONNECTIONS_PER_SERVER, region->head_net->hostname);
			region->head_net->rdma_conn[i] = crdma_client_create_connection_list_hosts( channel, region->head_net->IPs, 
					region->head_net->num_NICs, CLIENT_TO_SERVER_CONNECTION);
    }
  }
...
client_region * allocateclient_regions( char *ID, _Client_Regions * client_regions )
{
	...
	for (i=0; i< MAX_MAILBOX ; i++){
		...
		if( tmp_tu_client_region->head_net != NULL) {
			printf("RDMA_HEADNET %s %d\n\n",tmp_tu_client_region->head_net->hostname, tmp_tu_client_region->head_net->num_NICs);fflush(stdout);}
#if TU_RDMA
...
void Update_Client_Regions( _Client_Regions *client_regions, const struct String_vector *zk_regions)
{
	...
	if(client_regions->zk_regions == NULL){
		...
		for ( i = 0; i < zk_regions->count; i++ ){
			client_regions->tu_regions[i] = allocateclient_regions( client_regions->zk_regions->data[i], client_regions );
...
void Assign_Region_Min_Range( client_region *cli_tu_region , const char *min_range)
{
	if(cli_tu_region->inserted_tree == 1){
		...
		if ( client_compare(cli_tu_region->ID_region.minimum_range, (void *)min_range+sizeof(int),*(int*)min_range) == 0 ){
...
void Client_Print_Stat_Client_Regions( _Client_Regions *client_regions )
{
	...
	for ( i = 0; i < client_regions->zk_regions->count; i++ )
	{
		printf("Region %s Operations %llu\n",client_regions->tu_regions[i]->ID_region.IDstr,(unsigned long long) client_regions->tu_regions[i]->stat);
	}
	fflush(stdout);
}
...
{
	struct sigaction actions1;

	int num_threads = 1;
	int num_regions = 0;
	memset(&actions1, 0, sizeof(actions1));
	sigemptyset(&actions1.sa_mask);
	actions1.sa_flags = 0;
	actions1.sa_handler = sighand_term;
	sigaction(SIGUSR2,&actions1,NULL);
#if CLI_SIGNAL 
	{
...
#if CLI_RECV_THREADS
	{
		...
		for ( i = 0; i < num_regions; i ++ )	
		{
			for ( k = 0; k <  MAX_MAILBOX ; k++)
			{
				...
				{
					pthread_t thread2;
					pthread_create(&thread2, NULL, client_thread_receiving_messages_RDMA, client_regions->tu_regions[i]);
				}
				#endif
				}
			}
...
#else
	DPRINT("Client receive path configured set with spinning threads \n");
#endif
#else
	{
	char **servers_name;
...
#if CLI_RECV_THREADS
	{
		...
		{
			found = 0;
			for (j = 0; j < count_servers; j++ )
			{
				if ( strcmp(client_regions->tu_regions[i]->head_net->hostname, servers_name[j] ) == 0 )
				{
					found = 1;
					break;
				}
			...
			servers_name[ count_servers ] =  client_regions->tu_regions[i]->head_net->hostname;
			count_servers++;
...
#if TU_SEMAPHORE
struct tu_data_message *client_blocking_receive( client_region* cli_tu_region, struct tu_data_message *data_message , int next_mail )
{
	struct tu_data_message *reply_data_message;
	while(1){
