...
void Create_Regions(struct test_regions *a_region)
{
	...
	while ( ( ! a_region->done_min ) || ( ! a_region->done_max ) || ( ! a_region->done_size ) ||
		...
		printf("%d %d %d %d\n",a_region->done_min,a_region->done_max,a_region->done_size, a_region->done_head );
		sleep(1);
	}
}
/******************************************************************************
...
void create_replica_node_on_region( const char *value, const void *data ){
	...
	printf("\n\n***[%s:%s:%d] create_replica_on_region: zoo_acreate  %s and %s Chain %d %s ***\n\n",__FILE__,__func__,__LINE__,(char*) value, (char*)chainname, nchain, region->replicas_hostname[nchain]);
        zoo_acreate(   zh,
                       value,
                       region->replicas_hostname[nchain],
                       strlen((char*)region->replicas_hostname[nchain]) + 1,
                       &ZOO_OPEN_ACL_UNSAFE,
                       0,
                       create_replica_node_on_region_completion,
                       data );
...
{       
	struct test_regions *a_region;
	a_region = (struct test_regions *)data;
        printf("[%s:%s:%d] zoo_acreate  %s and %s\n",__FILE__,__func__,__LINE__,(char*) value, (char*)a_region->ID_region.IDstr);
...
void create_replica_node_on_server( const char *value, const void *data){
	...
	a_region = (struct test_regions *)data;
	path = make_path( 6, TUZK_SERVERS, "/", a_region->replicas_hostname[nchain], TUZK_REPLICAS , "/", a_region->ID_region.IDstr );
	printf("[%s:%s:%d] zoo_acreate  %s and %s path is %s nchain %d\n",__FILE__,__func__,__LINE__,(char*)value,(char*)a_region->ID_region.IDstr, path, nchain);
...
void Assign_Replicas_Server(struct test_regions *a_region)
{
	...
	if ( a_region->n_replicas == 0 ) return;
	for( i = 0; i < a_region->n_replicas ; i++ )
	{
		char chainname[256];
		sprintf( chainname, "%d", i );
...
void create_head_node_on_region( const char *value, const void *data ){
	...
	region = (struct test_regions*) data;
        ...
        zoo_acreate(   zh,
                       value,
                       region->head_hostname,
                       strlen((char*)region->head_hostname) + 1,
                       &ZOO_OPEN_ACL_UNSAFE,
                       0,
                       create_head_node_on_region_completion,
                       data );
...
{       
	...
	region = (struct test_regions*) data;
        ...
        zoo_acreate(   zh,
                       value,
                       region->tail_hostname,
                       strlen((char*)region->tail_hostname) + 1,
                       &ZOO_OPEN_ACL_UNSAFE,
                       0,
                       create_tail_node_on_region_completion,
                       data );
...
void create_chain_node_on_region(const char *value, const void *data){
	...
	zoo_acreate(zh, value, nchain, strlen(nchain) + 1, &ZOO_OPEN_ACL_UNSAFE, 0, create_chain_node_on_region_completion, data);
}
...
void create_min_key_node_on_region( const char *value, const void *data )
{
	...
	memcpy(min_range_to_string,(a_region->ID_region.minimum_range+sizeof(int)),*(int *)a_region->ID_region.minimum_range);
	printf("[%s:%s:%d] updating region's minimum key to: %s\n",__FILE__,__func__,__LINE__,min_range_to_string);
	zoo_acreate(zh,
		value,
		min_range_to_string,
		strlen(min_range_to_string) + 1,
		&ZOO_OPEN_ACL_UNSAFE,
		0,
		create_min_key_node_on_region_completion,
		data );
...
void create_max_key_node_on_region( const char *value, const void *data ){
	...
	memcpy(max_range_to_string,a_region->ID_region.maximum_range+sizeof(int),*(int*)a_region->ID_region.maximum_range);
        printf("[%s:%s:%d] updating region's maximum key to %s\n",__FILE__,__func__,__LINE__,max_range_to_string);
        zoo_acreate(   zh,
                       value,
                       max_range_to_string,
                       strlen(max_range_to_string) + 1,
                       &ZOO_OPEN_ACL_UNSAFE,
                       0,
                       create_max_key_node_on_region_completion,
                       data );
...
void server_aexist_region_node( struct test_regions *region )
{
    ...
    zoo_aexists(  zh,
            path,
            0,
            server_aexist_region_node_completion,
            (void *)region);
    free( path );
}
//..............................................................................
	...
	while( i < argc )
	{
		...
		else if ( strcmp(argv[i], PARA_MINKEY ) == 0 ) 
		{
			...
			*(int *)region->ID_region.minimum_range = strlen(argv[i]);
			...
			printf("[%s:%s:%d] argument %s %s\n",__FILE__,__func__,__LINE__,argv[i-1],region->ID_region.minimum_range+sizeof(int));
		}
		else if ( strcmp(argv[i], PARA_MAXKEY ) == 0 ) 
		{
			...
			*(int *)region->ID_region.maximum_range = strlen(argv[i]);
			...
			printf("[%s:%s:%d] Arg %s %s\n",__FILE__,__func__,__LINE__,argv[i-1], region->ID_region.maximum_range+sizeof(int));
		}
