...
{
	client_regions->zh = zookeeper_init(hostPort, client_main_watcher, 15000, 0, (void *)client_regions, 0);
	return errno;
}
...
void client_get_regions_num(_Client_Regions *client_regions)
{
	...
	int rc;
	rc = zoo_wget_children(client_regions->zh, TUZK_REGIONS, NULL, NULL, &strings);
	switch (rc) {
	case ZOK:
...
void cli_get_min_key_node_on_region(const char *value, const void *data)
{
	...
	/*gesalous, removed watcher*/
	rc = zoo_wget(cli_tu_region->parent->zh, value, NULL, NULL, buffer, &buffer_len, &stat);
	switch (rc) {
	case ZCONNECTIONLOSS:
...
void client_aexist_min_key_regions(client_region *cli_tu_region)
{
	...
	zoo_awexists(cli_tu_region->parent->zh, path, NULL, (void *)cli_tu_region,
		     client_aexist_min_key_regions_completion, (void *)cli_tu_region);
	free(path);
}
...
void client_get_Max_Key_region(client_region *cli_tu_region)
{
	...
	rc = zoo_wget(cli_tu_region->parent->zh, path, NULL, NULL, buffer, &buffer_len, &stat);
	switch (rc) {
	case ZCONNECTIONLOSS:
...
void client_aexist_max_key_regions(client_region *cli_tu_region)
{
	...
	/*gesalous, removed watcher*/
	zoo_awexists(cli_tu_region->parent->zh, path, NULL, (void *)cli_tu_region,
		     client_aexist_max_key_regions_completion, (void *)cli_tu_region);
	free(path);
}
...
void client_aexist_head_node_on_regions(client_region *cli_tu_region)
{
	...
	/*gesalous, removed watcher*/
	zoo_awexists(cli_tu_region->parent->zh, path, NULL, (void *)cli_tu_region,
		     client_aexist_head_node_on_regions_completion, (void *)cli_tu_region);
	free(path);
}
...
void client_get_Head_Data_Mailbox_region(client_region *cli_tu_region)
{
	...
	rc = zoo_wget_children(cli_tu_region->parent->zh, path, NULL, NULL, &strings);
	switch (rc) {
	case ZCONNECTIONLOSS:
...
void client_set_check_alive_server_of_region(client_region *cli_tu_region)
{
	...
	zoo_aset(cli_tu_region->parent->zh, path, "1", strlen((char *)"1") + 1, -1,
		 client_set_check_alive_server_of_region_completion, (void *)cli_tu_region);
	free(path);
}
...
void client_set_check_alive_server_of_region_completion(int rc, const struct Stat *stat, const void *data)
{
	...
	default:
		log_fatal(("Something went wrong when running client_set_check_alive_server_of_region."));
		break;
	}
}
...
void client_get_IP_head_region_completion(int rc, const struct String_vector *strings, const void *data)
{
	...
	case ZNONODE:
		//	client_get_IP_head_region( cli_tu_region );
		log_fatal(("Something went wrong when get_IP_head_region: %s", rc2string(rc)));
		break;
	default:
		log_fatal(("Something went wrong when get_IP_head_region: %s", rc2string(rc)));
		break;
...
void client_get_servers(_Client_Regions *client_regions)
{
	...
	rc = zoo_wget_children(client_regions->zh, TUZK_SERVERS, NULL, NULL, &strings);
	switch (rc) {
	case ZCONNECTIONLOSS:
	case ZOPERATIONTIMEOUT:
		client_get_servers(client_regions);
...
void client_aexist_servers(_Client_Regions *client_regions)
{
	...
	/*gesalous, removed watcher*/
	zoo_awexists(client_regions->zh, TUZK_SERVERS, NULL, (void *)client_regions, client_aexist_servers_completion,
		     (void *)client_regions);
}
...
void client_get_IP_server(_cli_tu_network_data *net_data)
{
	...
	rc = zoo_wget_children(client_regions->zh, path, NULL, NULL, &strings);
...
void client_get_root_completion(int rc, const struct String_vector *strings, const void *data)
{
	...
	default:
		log_fatal("GET_ROOT Something went wrong when get_root: %s", rc2string(rc));
		break;
	}
}
