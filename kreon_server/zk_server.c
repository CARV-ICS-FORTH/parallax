...
void Create_server_ID_in_Servers(void){
	char *path;
	printf("\n[%s:%s:%d] registering my hostname:%s to zookeeper\n\n",__FILE__,__func__,__LINE__,tuzk_S.net.hostname);
...
void create_data_nodes_server( const char *value, const void *data ){
	...
	switch (rc) {
		...
		case ZNODEEXISTS:
			...
			break;
		default:
			LOG_ERROR(("Something went wrong when creating data nodes on /server/hostname."));
			break;
	}
}
...
void aliveservers_exists_watcher (zhandle_t *zh, int type, int state, const char *path, void *watcherCtx) {
	...
	} else if ( type == ZOO_CHANGED_EVENT )
	{
		...
		printf("[%s:%s:%d] watched event: ",__FILE__,__func__,__LINE__,type2string(type));
}
...
void get_check_alive(void){
	...
	rc = zoo_wget( tuzk_S.zh,
			path,
			get_check_alive_watcher,
			NULL,
			buffer,
			&buffer_len,
			&stat);
...
void get_regions(void){
	...
	/*zoo_awget_children( tuzk_S.zh,
		...
		NULL);*/
	rc = zoo_wget_children(tuzk_S.zh,
			path,
			get_regions_watcher,
			NULL,
			&strings);
	...
	switch (rc){
		...
		case  ZNONODE:
			...
			break;
		default:
			LOG_ERROR(("Something went wrong when checking check_alive: %s", rc2string(rc)));
			break;
	}
	if(strings.count)
...
void zoo_create_server_regions( const char *value, const void *data ){
	...
	switch (rc) {
		...
		default:
			LOG_ERROR(("Something went wrong when creating data nodes on /server/hostname/regions."));
			break;
	}
}
...
default:
LOG_ERROR(("Something went wrong when checking replicas: %s", rc2string(rc)));
break;
}
if(strings->count){
...
void zoo_create_server_replicas( const char *value, const void *data ){
	...
	switch (rc) {
		...
		default:
			LOG_ERROR(("Something went wrong when creating data nodes on /server/hostname/replicas."));
			break;
	}
}
...
void zoo_create_server_NICs( const char *value, const void *data )
{
	...
	switch (rc) {
		...
		default:
			printf("[%s:%s:%d] Something went wrong when creating data nodes on /server/hostname/replicas rc = %d",__FILE__,__func__,__LINE__,rc);
...
void create_parent_server( void ){
	...
	switch (rc) {
		...
		case ZNODEEXISTS:
			...
			break;
		default:
			LOG_ERROR(("Something went wrong when running for master: %s", rc2string(rc)));
			break;
	}
}
//.............................................................................
...
LOG_ERROR(("Something went wrong when running for Min Key of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
break;
}
}*/
...
/*void server_get_max_key_node_on_region_completion(int rc, const char *value, int value_len, const struct Stat *stat, const void *data) {
	...
	LOG_ERROR(("Something went wrong when running for Max Key of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
	break;
	}
	}*/
...
/*void server_get_size_node_on_region_completion(int rc, const char *value, int value_len, const struct Stat *stat, const void *data) {
	...
	LOG_ERROR(("Something went wrong when running for Size of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
	break;
	}
	}*/
...
void server_get_StorageDevice_region( _tucana_region_S *server_tu_region )
{
	...
	if(server_tu_region->replica_type == REPLICA_HEAD)
		path = make_path( 7 , TUZK_SERVERS , "/", tuzk_S.net.hostname, TUZK_REGIONS, "/" , server_tu_region->ID_region.IDstr, TUZK_DEVICE );
	else
		path = make_path( 7 , TUZK_SERVERS , "/", tuzk_S.net.hostname, TUZK_REPLICAS, "/" , server_tu_region->ID_region.IDstr, TUZK_DEVICE );
...
/*void server_get_storage_device_node_on_region_completion(int rc, const char *value, int value_len, const struct Stat *stat, const void *data) 
	...
	LOG_ERROR(("Something went wrong when running for StorageDevice of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
	break;
	}
	}*/
...
void server_get_storage_device_node_on_region( const char *value, const void *data ){
	...
	switch (rc) {
		...
		default:
			server_aexist_storage_device_regions( server_tu_region );
			printf("[%s:%s:%d] Something went wrong when running for StorageDevice of %s %s\n",__FILE__,__func__,__LINE__,server_tu_region->ID_region.IDstr ,  rc2string(rc));
			break;
	}
}
/*
...
/*void server_get_offset_device_node_on_region_completion(int rc, const char *value, int value_len, const struct Stat *stat, const void *data) 
	...
	LOG_ERROR(("Something went wrong when running for Offset_Device of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
	break;
	}
	}*/
...
void server_aexist_offset_device_regions( _tucana_region_S *server_tu_region ){
	...
	path = make_path( 7 , TUZK_SERVERS , "/", tuzk_S.net.hostname, TUZK_REGIONS, "/" , server_tu_region->ID_region.IDstr, TUZK_OFFSET );
	/*zoo_awexists(  tuzk_S.zh,
		path,
		server_aexist_offset_device_regions_watcher,
		(void *)server_tu_region,
...
default:
LOG_ERROR(("Something went wrong when running for OffsetDevice of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
break;
}
}*/
...
/*void server_get_chain_node_on_region_completion(int rc, const char *value, int value_len, const struct Stat *stat, const void *data) 
	...
	LOG_ERROR(("Something went wrong when running for Chain of %s %s\n" ,server_tu_region->ID_region.IDstr ,  rc2string(rc)));
	break;
	}
	}*/
...
/*void server_get_servers_completion ( int rc, const struct String_vector *strings, const void *data ){
	...
	LOG_ERROR(("GET_SERVERS Something went wrong when get_servers: %s", rc2string(rc)));
	break;
	}
	if(strings->count){
	printf("[%s:%s:%d] ************* ****************** cleaning ***************************************...\n",__FILE__,__func__,__LINE__);
...
retry:
	...
	rc = zoo_wget_children( server_regions->zh,
			value,
			server_get_IP_server_watcher,
			(void *)data,
			&strings);
	...
	switch (rc){
		...
		case  ZNONODE: //PILAR TO SOLVE
			//server_get_IP_server( net_data );
			printf("[%s:%s:%d] GET_REGIONS Something went wrong when IP regions: %s trying again",__FILE__,__func__,__LINE__,rc2string(rc));
			create_server_NICs();
			++tries;
			goto retry;
