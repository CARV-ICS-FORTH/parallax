...
retry:
	...
	if(region == NULL){
		DPRINT("FATAL NULL region for key %s\n",key);
		raise(SIGINT);
		exit(EXIT_FAILURE);
	}

	//DPRINT("region for key %s:%d is region with min %s:%d max %s:%d conn is %llu\n",key,key_size,region->ID_region.minimum_range+4,
	//	*(uint32_t*)region->ID_region.minimum_range, region->ID_region.maximum_range+4,*(uint32_t*)region->ID_region.maximum_range,region->head_net->rdma_conn[0]);
	return region;
}
...
int client_add_region(client_region* region)
{
	...
	if(kreon_regions_size > 0){
		while(start_idx <= end_idx){
			middle = (start_idx + end_idx)/2;
			ret = client_compare(kreon_regions[middle]->ID_region.minimum_range,region->ID_region.minimum_range+sizeof(uint32_t), *(uint32_t *)region->ID_region.minimum_range);
			if(ret == 0){
