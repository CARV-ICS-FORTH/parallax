...
{
	/*write log info*/
	if (handle->db_desc->big_log_head != NULL)
		handle->db_desc->commit_log->big_log_head =
			(segment_header *)ABSOLUTE_ADDRESS(handle->db_desc->big_log_head);
	else
		handle->db_desc->commit_log->big_log_head = NULL;
	if (handle->db_desc->big_log_tail != NULL)
		handle->db_desc->commit_log->big_log_tail =
			(segment_header *)ABSOLUTE_ADDRESS(handle->db_desc->big_log_tail);
	else
		handle->db_desc->commit_log->big_log_tail = NULL;
	handle->db_desc->commit_log->big_log_size = handle->db_desc->big_log_size;
				...
				uint64_t end_of_log, uint64_t bytes_to_pad, uint64_t segment_id)
{
	...
	volatile segment_header *disk_segment_header;
	...
	if (handle->db_desc->big_log_size == 0 ||
	    (handle->db_desc->big_log_size > 0 && handle->db_desc->big_log_size % BUFFER_SEGMENT_SIZE == 0)) {
		buffer_offset = 0;
	} else {
		buffer_offset = handle->db_desc->big_log_size % BUFFER_SEGMENT_SIZE;
	}

	buffer_bytes_to_write = end_of_log - handle->db_desc->big_log_size;
	...
	/****************** assert check *********************/
	if (handle->db_desc->big_log_tail != NULL && handle->db_desc->big_log_tail->segment_id != segment_id) {
		if (handle->db_desc->big_log_tail->segment_id + 1 != segment_id) {
			log_info(" No sequential segment ids last %llu current %llu db %s\n",
				 (LLU)handle->db_desc->big_log_tail->segment_id, (LLU)segment_id,
	...
	if (handle->db_desc->big_log_tail == NULL || handle->db_desc->big_log_tail->segment_id < segment_id) {
		...
		if (handle->db_desc->big_log_tail != NULL) {
			s_header->prev_segment = (segment_header *)ABSOLUTE_ADDRESS(handle->db_desc->big_log_tail);
		} else {
		...
		if (handle->db_desc->big_log_tail != NULL) {
			handle->db_desc->big_log_tail->next_segment =
				(segment_header *)((uint64_t)disk_segment_header - MAPPED);
		}

		handle->db_desc->big_log_tail = disk_segment_header;

		if (handle->db_desc->big_log_head == NULL) {
			log_info("initializing first segment for db %s\n", handle->db_desc->db_name);
			handle->db_desc->big_log_head = handle->db_desc->big_log_tail;
		}
	...
	} else if (handle->db_desc->big_log_tail->segment_id == segment_id) {
		disk_segment_header = handle->db_desc->big_log_tail;
	} else {
		log_info("FATAL id out of range\n");
		exit(EXIT_FAILURE);
	}

	handle->db_desc->big_log_size = end_of_log;
	...
	if (handle->db_desc->big_log_tail->segment_id != segment_id) {
