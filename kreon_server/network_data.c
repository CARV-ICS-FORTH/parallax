...
void Get_My_IP_Addresses( _tu_network_data *net )
{
	...
	for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next) {
		...
		if(family == AF_INET){
			...
			if(strncmp(addr,RDMA_IP_FILTER, strlen(RDMA_IP_FILTER)) == 0){
				printf("[%s:%s:%d] (gesalous hack accepts only %s prefix) Interface: %s\tAddress: %s\n",
			       __FILE__,__func__,__LINE__, RDMA_IP_FILTER,ifa->ifa_name, addr);
				n++;
			}
			/*</gesalous>*/
		}
	...
	for (ifa = ifaddr, i = 0 ; ifa != NULL; ifa = ifa->ifa_next) {
		...
		if (family == AF_INET ) {
			...
			addr = inet_ntoa(sa->sin_addr);
			if(strncmp(addr,RDMA_IP_FILTER, strlen(RDMA_IP_FILTER)) != 0){
				printf("[%s:%s:%d] Interface: %s\tAddress: %s\n",
					__FILE__,__func__,__LINE__, ifa->ifa_name, addr);
				continue;
			}
			/*</gesalous>*/
