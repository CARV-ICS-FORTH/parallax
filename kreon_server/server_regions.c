...
static inline char _DB_CLOSE_NOTIFY(_tucana_region_S * region)
{
	...
	do{
		...
		DPRINT("Region %s is closing active workers are %ld\n",region->ID_region.IDstr, value);
...
_tucana_region_S * allocate_tucana_regions(char * ID){
	...
	tmp_tu_region_S->chain_replication = NULL;
	tmp_tu_region_S->list_pending_replicated = NULL;
...
void Update_Server_Regions(const struct String_vector *zk_regions, char REGIONS_TYPE){
	...
	if(REGIONS_TYPE == PRIMARY){
		...
		if((*current_zk_regions == NULL ) && ( zk_regions->count == 0 )){
			DPRINT("no REPLICA regions yet :-(\n");
			return;
		}
	} else{
		DPRINT("FATAL unknown region type (PRIMARY or REPLICA)\n");
		exit(EXIT_FAILURE);
	}
	...
	*current_tu_regions = new_tu_regions;
	...
	for ( i = 0; i < old_zk_regions->count; i++ ){
		...
		if ( pos >= 0 ) {
			printf("[%s:%s:%d] region %s exists old_pos %d new_pos %d\n",__FILE__,__func__,__LINE__,old_zk_regions->data[i], i, pos);
...
void Server_Assign_Region_Min_Range( _tucana_region_S *S_tu_region , const char *min_range )
{
	if ( S_tu_region->inserted_tree == 1 ){
		if (memcmp(S_tu_region->ID_region.minimum_range+sizeof(int),min_range+sizeof(int), *(int *)S_tu_region->ID_region.minimum_range) == 0 ){
                        DPRINT("Assign_Region_Min_Range: New min_range is equal\n");
                        return;
                }
...
void Server_Assign_Region_Chain( _tucana_region_S *S_tu_region , const char *str_region_chain ){
	...
	DPRINT(" n_replicas = %d region replicas %d\n",n_replicas, S_tu_region->n_replicas);
...
void Server_Assign_StorageDevice_Region(_tucana_region_S *S_tu_region, const char *storage_device ){
	...
	printf("[%s:%s:%d] assigning StorageDeviceRegion %s %s\n",__FILE__,__func__,__LINE__,S_tu_region->ID_region.IDstr, storage_device);
...
void Server_Set_New_StorageDevice_Region( _tucana_region_S *S_tu_region ){
	...
	printf("[%s:%s:%d] assigning volume to region:%s\n",__FILE__,__func__,__LINE__,S_tu_region->ID_region.IDstr);
...
void region_group_membership_watcher(zhandle_t *zh, int type, int state, const char *path, void *watcherCtx)
{
  ...
  char *read_path, *write_path;
	...
	DPRINT("got event for path: %s, something changed! max group size %d group size %d children count %d\n",
			path,region->gmt.max_group_size, region->gmt.current_group_size, group_children.count);



	if(strstr(path, "group") != NULL && type == ZOO_CHILD_EVENT){
		if(group_children.count >= region->gmt.max_group_size){
			DPRINT("all servers joined :-)\n");
			...
			DPRINT("someone (server within the group) failed\n");
			region->db->db_desc->data_conn = NULL;
			...
			if(region->gmt.uuid  == min_id){
				DPRINT("starting chain reconfiguration\n");
        ...
        int idx = 1;
				...
				for(i=1;i<group_children.count;i++){
					read_path = make_path(7,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/",REGION_GROUP,"/",group_children.data[i]);
					buffer_len = 256;
				...
				region->n_replicas = 0;
				region->db = db_open(region->device,region->offset,region->size,db_name, O_CREATE_DB);
				region->db->db_desc->region_min_key = region->ID_region.minimum_range;
				free(db_name);
				/*configuration done upload to zk. Store old state, in case of a failure*/
				read_path = make_path(4,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/old_configuration");
				...
				if(rc == ZOK ){
					...
					zoo_wget_children(tuzk_S.zh, read_path,region_group_membership_watcher,region,&chains_children_1);
					free(read_path);

					for(i=0;i<chains_children_1.count;i++){
						if(strcmp(chains_children_1.data[i],"old_configuration")==0)
							continue;
						read_path = make_path( 5, TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/",chains_children_1.data[i]);
						write_path = make_path(5,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/old_configuration/",chains_children_1.data[i]);

						buffer_len = 256;
						zoo_get(tuzk_S.zh, (const char *)read_path, 0, buffer, &buffer_len, &stat);
						zoo_create(tuzk_S.zh, write_path, buffer,buffer_len, &ZOO_OPEN_ACL_UNSAFE, 0, NULL, 0);
						free(read_path);
						free(write_path);
					}
				} else {
					DPRINT("FATAL failed to create %s there is already a valid previous configuration XXX TODO XXX rethink about this\n",read_path);
					exit(EXIT_FAILURE);
				}
				...
				for(i=0;i<chains_children_1.count;i++){
					if(strcmp(chains_children_1.data[i],"old_configuration")==0)
						continue;
					write_path = make_path(5,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/",chains_children_1.data[i]);
					zoo_delete(tuzk_S.zh,write_path,-1);
					free(write_path);
				}
				DPRINT("uploading new configuration ....\n");
				/*upload  new configuration*/
				char id[64];
				for(i=0;i<region->gmt.current_group_size;i++){
					sprintf(id,"%d",i);
					write_path = make_path(5,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/",id);
					...
					free(write_path);
					if(i==0){
						write_path = make_path(5,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/","head");	
						zoo_create(tuzk_S.zh, write_path, buffer,buffer_len, &ZOO_OPEN_ACL_UNSAFE, 0, NULL, 0);
						free(write_path);

					}
					if(i == region->gmt.current_group_size -1){
						write_path = make_path(5,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/","tail"); 
                                                zoo_create(tuzk_S.zh, write_path, buffer,buffer_len, &ZOO_OPEN_ACL_UNSAFE, 0, NULL, 0);
                                                free(write_path);
					}
				}
				...
				DPRINT("alive replicas now are %d\n",region->n_replicas);
				...
				DPRINT("Done! notify everybody\n");
				write_path = make_path(4,TUZK_REGIONS, "/" ,region->ID_region.IDstr,REGION_GROUP); 
				zoo_set(tuzk_S.zh, write_path, buffer, 4, -1);/*XXX TODO XXX add something meaningful?*/
				free(write_path);

			} else {
				DPRINT("waiting for chain reconfiguration\n");
		...
		if(region->gmt.state != WAITING_FOR_RECONFIGURATION){
			DPRINT("FATAL could not reconfigure, region is in wrong state\n");
			exit(EXIT_FAILURE);
		}
		...
		for(i=0;i<chains_children_2.count;i++){
			read_path = make_path(4,TUZK_REGIONS, "/" ,region->ID_region.IDstr,"/chains/",chains_children_2.data[i]);
			memset(buffer,0x00,128);
			buffer_len = 256;
			zoo_get(tuzk_S.zh, (const char *)read_path, 0, buffer, &buffer_len, &stat);
			free(read_path);
...
int ServerOpen_TucanaDB( _tucana_region_S *S_tu_region ){
  uint64_t size_device;
	...
	* see if we are the head or not and set the appropriate fields*/
	...
	int32_t rc = 0;
  DPRINT("retrieving metadata for replicated region %s\n",S_tu_region->ID_region.IDstr);
	...
	for(m = 0; m < children.count; m++){/*do not count head and tail, 0 is the primary, 1 ,2 etc*/
		...
		path = make_path(6,TUZK_REGIONS, "/" ,S_tu_region->ID_region.IDstr,TUZK_CHAINS,"/",children.data[m]);
		memset(buffer,0x00,128);
		buffer_len = 128;
		if(zoo_get(tuzk_S.zh, (const char *)path, 0, buffer, &buffer_len, &stat)!= ZOK){
			DPRINT("FATAL cannot get status %s for region %s\n",path,S_tu_region->ID_region.IDstr);
			exit(EXIT_FAILURE);
		}
		...
		DPRINT("child = %s num %d data %s path %s buffer_len %d\n",children.data[m], idx, buffer, path, buffer_len);
		free(path);
		S_tu_region->replicas_hostname[idx] = strdup(buffer);
		++chain_size;
	}
	S_tu_region->n_replicas = chain_size;
	DPRINT("region replicas %d\n",S_tu_region->n_replicas);
	...
	char self[128];/*uuid constructed by hashing(hostname+ts)*/
        ...
        assert(strlen(tuzk_S.net.hostname) <= MAX_HOSTNAME);
         ...
         do{
                 rc = zoo_create(tuzk_S.zh, path, data,strlen(tuzk_S.net.hostname)+1,&ZOO_OPEN_ACL_UNSAFE,ZOO_SEQUENCE|ZOO_EPHEMERAL,self,128);
                 if(rc == ZOK)
                         break;
	...
	path = make_path(4,TUZK_REGIONS, "/" ,S_tu_region->ID_region.IDstr,REGION_GROUP);
	DPRINT("left watcher for possible children change: %s for failure detection purposes\n",path);
	...
	if(S_tu_region->replica_type == REPLICA_HEAD){
		if(S_tu_region->db != NULL){
			DPRINT("Warning db already open XXX TODO XXX, fix this\n");
			S_tu_region->ready_db = 1;
			return 1;
		} else {
			DPRINT("Opening primary db: %s\n",S_tu_region->ID_region.IDstr);
			S_tu_region-> db = db_open( S_tu_region->device, 0 , size_device, S_tu_region->ID_region.IDstr, O_CREATE_DB );
			S_tu_region->db->db_desc->region_min_key = S_tu_region->ID_region.minimum_range;
    }
	}else{
		 if(S_tu_region->db != NULL){
			DPRINT("Warning db already open XXX TODO XXX, fix this\n");
			...
			DPRINT("Opening backup db: %s\n",S_tu_region->ID_region.IDstr);
			if(S_tu_region->db != NULL){
				DPRINT("Warning db already open XXX TODO XXX, fix this\n");
				S_tu_region->ready_db = 1;
				return 1;
			} else
				S_tu_region->db = db_open( S_tu_region->device, 0 , size_device, S_tu_region->ID_region.IDstr, O_CREATE_REPLICA_DB );
				S_tu_region->db->db_desc->region_min_key = S_tu_region->ID_region.minimum_range;
      }
    }
	...
	if ( S_tu_region-> db == NULL ){
		DPRINT("ERROR Region %s failed to open Tucana DB %s\n",S_tu_region->ID_region.IDstr, S_tu_region->device);
		return 0;
	}
	S_tu_region->ready_db = 1;
...
void Server_Set_Position_Replica_of_Region( _tucana_region_S *S_tu_region )
{
	...
	for( i = 0; i < S_tu_region->n_replicas ; i++ ){
		...
		if(S_tu_region->replicas_hostname[i] == NULL ){
			DPRINT("FATAL ERROR found null hostname at position %d?????????????????????\n",i);
			exit(EXIT_FAILURE);
		}else{
	...
	for( i = 0; i < S_tu_region->n_replicas ; i++ ){
		DPRINT("Comparing myhostname %s with replicas %s\n",tuzk_S.net.hostname,  S_tu_region->replicas_hostname[i]);
		if(strcmp( S_tu_region->replicas_hostname[i],  tuzk_S.net.hostname ) == 0){
			if(i == 0){
				S_tu_region->replica_type = REPLICA_HEAD;
				DPRINT("I am HEAD replica[%d] =  %s\n", i,S_tu_region->replicas_hostname[i]);
			}
			else if(i == (S_tu_region->n_replicas - 1 )){
				DPRINT("I am TAIL replica[%d] =  %s\n", i,S_tu_region->replicas_hostname[i]);
			...
			if((i+1) < S_tu_region->n_replicas){
				...
				S_tu_region->replica_next_net = Server_FindHostname_Servers( S_tu_region->hostname_replica_next, &tuzk_S.servers) ;
			}
			return;
		}
	}
...
void Server_Waiting_DBs_are_Open( _RegionsSe *aux_regions_S )
{
  ...
  int open_regions_num;
	...
	DPRINT("waiting for at least for 1 region (PRIMARY OR REPLICA) to join\n");
