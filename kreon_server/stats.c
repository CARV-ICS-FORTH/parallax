...
void stats_init(int worker_threads) {
	...
	sum_scan_length = (uint32_t*) malloc(worker_threads * sizeof(uint32_t)); operations = (uint32_t*) malloc(worker_threads * sizeof(uint32_t));
...
static void* stats_reporter_thread(void* args) {
	...
	do {
		...
		fprintf(stdout, "%lu Sec %.2f Ops/sec\n", seconds_passed, (ops_at_curr_second - ops_at_last_second)/(double)SLEEP_DURATION_TIMESPEC.tv_sec);
