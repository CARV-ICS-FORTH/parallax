...
inline void push_stack(stack* marks,void * addr){
...
void move_kv_pairs_to_new_segment(volume_descriptor * volume_desc,
...
int8_t find_deleted_kv_pairs_in_segment(volume_descriptor * volume_desc,
					db_descriptor *  db_desc,char* log_segment,
					stack* marks){
	...
	while(size_of_log_segment_checked < log_data_without_metadata
	      && key->size != 0 && remaining_space >= 10 ){
...
inline void fix_nodes_in_log(volume_descriptor* volume_desc, db_descriptor* db_desc,
			     log_segment* prev_node, log_segment* curr_node){
	if(prev_node){
		prev_node->metadata.next_block = curr_node->metadata.next_block;
		free_block(volume_desc, curr_node, BUFFER_SEGMENT_SIZE, -1);
	}else
		db_desc->g_first_kv_log = (block_header *)REAL_ADDRESS((uint64_t)curr_node->metadata.next_block);

}

void iterate_log_segments(db_descriptor* db_desc,volume_descriptor* volume_desc,stack* marks){
	log_segment *   last_segment = (log_segment*)db_desc->g_last_kv_log;
	log_segment *	log_node     = (log_segment*)db_desc->g_first_kv_log;
	log_segment *	prev_node    = NULL;


	/* We are in the first segment of the log and is not yet full! */
	if(log_node->metadata.next_block == NULL){
		log_debug("We reached at the last log segment");
		return;
	}

	while(REAL_ADDRESS(log_node->metadata.next_block) != last_segment){
		int8_t ret = find_deleted_kv_pairs_in_segment(volume_desc,db_desc,
						       log_node->data,marks);
		...
		prev_node = log_node;
		log_node = (log_segment *)REAL_ADDRESS((uint64_t)log_node->metadata.next_block);
	}

	/* The log had multiple nodes and we reached at the last one! */
	if(REAL_ADDRESS(log_node->metadata.next_block) == last_segment){
		log_debug("We reached at the last log segment");
		return;
	}
...
void * gc_log_entries(void* v_desc){
	...
	while (1){
		...
		region = getFirst(volume_desc->open_databases);
