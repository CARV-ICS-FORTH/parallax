...
void * createEmptyNode(allocator_descriptor * allocator_desc, db_handle* handle,
		       nodeType_t type, char allocation_code);
...
uint8_t __delete_from_leaf(delete_request* req,index_node* parent,
			   leaf_node* leaf,struct splice* key);

void delete_key_value(db_descriptor * db_desc,leaf_node* leaf, int pos);

uint8_t transfer_node_to_neighbor_index_node(index_node* curr,index_node* parent,
					     rotate_data* siblings,
					     delete_request* req);

void __find_left_and_right_siblings(index_node* parent,void* key,
				    rotate_data* siblings);
...
int8_t check_for_underflow_in_leaf(leaf_node* leaf,rotate_data* siblings,
				   delete_request* req);

int8_t merge_with_index_neighbor(index_node* curr,index_node* parent,
				  rotate_data* siblings,delete_request* req);
