...
static void comp_get_space(struct comp_level_write_cursor *w_cursor, uint32_t height, nodeType_t type)
{
	...
	case leafRootNode: {
		...
		if (remaining_space < level_leaf_size) {
			...
			if (w_cursor->segment_offt[height] != 0) {
				...
				comp_write_segment(w_cursor->segment_buf[0], w_cursor->last_segment_btree_level_offt[0],
						   0, SEGMENT_SIZE, w_cursor->fd);
			}
		...
		comp_init_dynamic_leaf(w_cursor->last_leaf);
	...
	case rootNode: {
		...
		if (remaining_space < index_node_get_size()) {
			...
			if (w_cursor->segment_offt[height] != 0) {
				...
				comp_write_segment(w_cursor->segment_buf[height],
...
void comp_init_write_cursor(struct comp_level_write_cursor *w_cursor, struct db_handle *handle, int level_id, int fd)
{
	memset(w_cursor, 0, sizeof(struct comp_level_write_cursor));
	w_cursor->level_id = level_id;
	w_cursor->tree_height = 0;
	w_cursor->fd = fd;
	w_cursor->handle = handle;

	comp_get_space(w_cursor, 0, leafNode);
	...
	for (int i = 1; i < MAX_HEIGHT; ++i) {
		comp_get_space(w_cursor, i, internalNode);
...
void comp_close_write_cursor(struct comp_level_write_cursor *w_cursor)
{
	...
	for (int32_t i = 0; i < MAX_HEIGHT; ++i) {
		...
		if (i == w_cursor->tree_height) {
			...
			uint32_t offt =
				comp_calc_offt_in_seg(w_cursor->segment_buf[i], (char *)w_cursor->last_index[i]);
		...
		comp_write_segment(w_cursor->segment_buf[i], w_cursor->last_segment_btree_level_offt[i], 0,
...
static int comp_append_medium_L1(struct comp_level_write_cursor *w_cursor, struct comp_parallax_key *in_key,
				 struct comp_parallax_key *out_key)
{
	...
	if (db_desc->medium_log.head_dev_offt == 0 && db_desc->medium_log.tail_dev_offt == 0 &&
		...
		comp_init_medium_log(w_cursor->handle->db_desc, w_cursor->level_id, 1);
	}
	...
	ins_req.key_value_buf = in_key->kv_inplace;
	...
	struct kv_splice *kv_inplace = (struct kv_splice *)in_key->kv_inplace;
	if (get_key_size(kv_inplace) >= PREFIX_SIZE)
		memcpy(out_key->kv_sep.prefix, get_key_offset_in_kv(kv_inplace), PREFIX_SIZE);
	else {
		memset(out_key->kv_sep.prefix, 0x00, PREFIX_SIZE);
		memcpy(out_key->kv_sep.prefix, get_key_offset_in_kv(kv_inplace), get_key_size(kv_inplace));
	}
	out_key->kv_sep.dev_offt = (uint64_t)log_location;
	out_key->kv_category = MEDIUM_INLOG;
	out_key->kv_type = KV_INLOG;
	out_key->kv_inlog = &out_key->kv_sep;
	out_key->tombstone = 0;

	return 1;
}

void comp_append_entry_to_leaf_node(struct comp_level_write_cursor *cursor, struct comp_parallax_key *kv)
{
