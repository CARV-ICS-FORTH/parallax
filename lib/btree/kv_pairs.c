...
#include <assert.h>
#include <string.h>
inline int32_t get_key_size(struct splice *kv_pair)
{
	return kv_pair->key_size;
}

// cppcheck-suppress unusedFunction
inline int32_t get_key_size_with_metadata(struct splice *kv_pair)
{
	return sizeof(kv_pair->key_size) + kv_pair->key_size;
}

inline int32_t get_value_size(struct splice *kv_pair)
{
	return is_tombstone_kv_pair(kv_pair) ? 0 : kv_pair->value_size;
}

// cppcheck-suppress unusedFunction
inline int32_t get_value_size_with_metadata(struct splice *kv_pair)
{
...
{
	int32_t kv_pair_metadata_size = sizeof(struct splice);
	return kv_pair_metadata_size;
}

inline int32_t get_kv_size(struct splice *kv_pair)
{
	return get_key_size(kv_pair) + get_value_size(kv_pair) + get_kv_metadata_size();
}

inline void set_key_size(struct splice *kv_pair, int32_t key_size)
{
	kv_pair->key_size = key_size;
}

inline void set_key(struct splice *kv_pair, char *key, int32_t key_size)
{
	kv_pair->key_size = key_size;
	memcpy(get_key_offset_in_kv(kv_pair), key, key_size);
}

inline void set_value_size(struct splice *kv_pair, int32_t value_size)
{
	kv_pair->value_size = value_size;
}

inline void set_value(struct splice *kv_pair, char *value, int32_t value_size)
{
...
inline char *get_value_offset_in_kv(struct splice *kv_pair, int32_t key_size)
{
...
inline bool is_tombstone_kv_pair(struct splice *kv_pair)
{
