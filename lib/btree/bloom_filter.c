...
bool pbf_persist_bloom_filter(struct pbf_desc *bloom_filter)
{
	...
	int bloom_file_desc =
		USE_DIRECT_IO ?
			...
			open(full_bloom_name, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC, S_IWUSR | S_IRUSR);
					  ...
					  uint64_t bloom_file_hash)
{
	...
	int bloom_file_desc = USE_DIRECT_IO ? open(full_bloom_name, O_RDONLY | O_CREAT | O_DIRECT | O_CLOEXEC, 0600) :
					      open(full_bloom_name, O_RDONLY | O_CREAT | O_CLOEXEC, 0600);
