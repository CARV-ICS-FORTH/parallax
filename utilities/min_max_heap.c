...
{
	int smallest = (LCHILD(i) < hp->size && _cmp_heap_nodes(hp, &hp->elem[LCHILD(i)], &hp->elem[i])) ? LCHILD(i) : i ;
...
int _cmp_heap_nodes(minHeap *hp, heap_node * nd_1, heap_node * nd_2)
{
	...
	if(ret == 0) /* duplicatelicate detected smallest level_id wins, needs more thinking */
	{
		...
		else
		{
			...
			printf("[%s:%s:%d] active tree = %d nd_1 level_id = %d nd_2 level_id = %d\n",__FILE__,__func__,__LINE__,hp->active_tree,nd_1->level_id,nd_2->level_id);
			printf("[%s:%s:%d] key1 %s key2 %s\n",__FILE__,__func__,__LINE__,(char *)nd_1->data+4, (char *)nd_2->data+4);
			exit(-1);
		}
	}
	return ret;
