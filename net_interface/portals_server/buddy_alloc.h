...
static inline unsigned char compare_with_internal_position(unsigned char *bitset, struct internal_position pos,
							   size_t value);
...
static inline struct internal_position buddy_tree_internal_position_tree(struct buddy_tree *t,
									 struct buddy_tree_pos pos)
{
	struct internal_position p;
	size_t total_offset, local_index;
...
static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree *t, uint8_t target_depth)
{
	...
	while (current_depth != target_depth) {
		...
		} else if (compare_with_internal_position(tree_bits, right_internal,
...
static void buddy_tree_debug(struct buddy_tree *t, struct buddy_tree_pos pos, size_t start_size)
{
	...
	do {
		...
		size_t pos_size = start_size >>
				  ((buddy_tree_depth(state.current_pos) - 1u) % ((sizeof(size_t) * CHAR_BIT) - 1));
		...
		BUDDY_PRINTF("pos index: %zu pos depth: %zu status: %zu bitset-len: %zu bitset-at: %zu",
			     state.current_pos.index, state.current_pos.depth, pos_status, pos_internal.local_offset,
			     pos_internal.bitset_location);
	...
	do {
		...
		if (violated) {
			...
			BUDDY_PRINTF("current: %zu left %zu right %zu max %zu\n", current_status, left_child_status,
				     right_child_status, current_internal.local_offset);
		}
...
static const uint8_t bitset_char_mask[8][8] = {
	{ 1, 3, 7, 15, 31, 63, 127, 255 }, { 0, 2, 6, 14, 30, 62, 126, 254 }, { 0, 0, 4, 12, 28, 60, 124, 252 },
	{ 0, 0, 0, 8, 24, 56, 120, 248 },  { 0, 0, 0, 0, 16, 48, 112, 240 },  { 0, 0, 0, 0, 0, 32, 96, 224 },
