...
def generate_script_lines(
    path, num_of_ranges, max_range, region_size, servers, replication
):
...
def main(argv):
    ...
    parser.add_argument(
        "create_regions_exe", help="Absolute path to Kreon's region creation executable"
    )
    ...
    parser.add_argument(
        "server_id",
        nargs="+",
        help="White space delimited list of hostname-<port> for each Kreon server",
    )
    parser.add_argument(
        "-r",
        "--replication_enabled",
        action="store_true",
        help="Assign a replica server to each created region",
    )
    parser.add_argument(
        "-o", "--output_file", help="Write the generated script to the output file"
    )
    ...
    lines = generate_script_lines(
        args.create_regions_exe,
        args.regions,
        args.max_key,
        args.storage,
        args.server_id,
        args.replication_enabled,
    )
