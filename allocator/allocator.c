...
int32_t volume_init(char *dev_name, int64_t start, int64_t size, int typeOfVolume)
{
	...
	for(i=0;i<bitmap_size_in_blocks;i++){
		...
		if(pwrite(fd, buffer, DEVICE_BLOCK_SIZE, offset) != DEVICE_BLOCK_SIZE){
			fprintf(stderr, "Function = %s, code = %d,  ERROR = %s\n",
				__func__, errno, strerror(errno));
	...
	if(pwrite(fd, &super_index, sizeof(superindex), offset) != sizeof(superindex)){
		fprintf(stderr, "Function = %s, code = %d,  ERROR = %s\n",__func__, errno, strerror(errno));
		return -1;
	}
	...
	if(pwrite(fd, dev_superblock, sizeof(superblock), start) != sizeof(superblock)){
		fprintf(stderr, "Function = %s, code = %d,  ERROR = %s\n",__func__, errno, strerror(errno));
		return -1;
	}
...
void destroy_volume_node(NODE *node)
{
	...
	destroyList(volume_desc->open_databases);
	free(volume_desc);
}
/**
...
void volume_close(volume_descriptor *volume_desc)
{
	...
	if(findElement(mappedVolumes, volume_desc->volume_id) == NULL)
	{
	...
	removeElement(mappedVolumes, volume_desc);
...
void* allocate_segment(void * _handle, uint64_t num_bytes, int level_id, char allocation_code){
	...
	if(allocation_code == NEW_LEVEL_0_TREE){
		printf("[%s:%s:%d] initializing new level-0 tree: %d\n",__FILE__,__func__,__LINE__,level_id);
		...
		*(uint64_t *)(segments[level_id*3]+sizeof(uint64_t)) = (uint64_t)BUFFER_SEGMENT_SIZE;
		...
		printf("[%s:%s:%d] initializing new level-1 tree: %d\n",__FILE__,__func__,__LINE__,level_id);
	...
	/*level-0 allocation*/
		...
		//printf("%"PRIu64"\n", ((db_handle *)_handle)->db_desc->zero_level_memory_size);
		MUTEX_UNLOCK(&volume_desc->allocator_lock);
		return addr;
...
void *allocate(void * _volume_desc, uint64_t num_bytes, int unused, char allocation_code)
{
	...
	while (1)
	{
		if((uint64_t) word_address == (uint64_t) 0xFFFFFFFFFFFFFFFF)/*reached end of bitmap*/
		{
			...
			{
				fprintf(stderr,"[%s:%s:%d] device out of space allocation request size was %llu max_tries %d\n",__FILE__,__func__,__LINE__,(LLU) num_bytes, MAX_ALLOCATION_TRIES);
				raise(SIGINT);
				return NULL;
			}
			else
...
void allocator_init(volume_descriptor * volume_desc)
{
	...
	{ /* apapag: some debugging prints */
		fprintf(stderr, "IN_LENGTH = %lu\n", IN_LENGTH);
		fprintf(stderr, "sizeof(index_entry) = %lu\n", sizeof(index_entry));
		fprintf(stderr, "sizeof(index_node) = %lu\n", sizeof(index_node));
		fprintf(stderr, "LN_LENGTH = %lu\n", LN_LENGTH);
		fprintf(stderr, "sizeof(leaf_node) = %lu\n", sizeof(leaf_node));

		static_assert(sizeof(index_node) == 4096, "Code relies on index_node is 4096");
		static_assert(sizeof(leaf_node) == 4096, "Code relies on index_node is 4096");
	}
...
void clean_log_entries(void *v_desc)
{
	...
	printf("[%s:%s:%d] Starting cleaner for volume id: %s\n",__FILE__,__func__,__LINE__,(char *) volume_desc->volume_id);
	while (1){
		...
		rc = MUTEX_LOCK(&volume_desc->mutex);
		rc = pthread_cond_timedwait(&(volume_desc->cond), &(volume_desc->mutex), &ts);

		if(rc == 0){/*cleaner signaled due to space pressure*/
		...
		MUTEX_UNLOCK(&volume_desc->allocator_lock);
		/*unlock, to go to sleep*/
		MUTEX_UNLOCK(&volume_desc->mutex);
		/*snapshot check*/
		if((get_timestamp() - volume_desc->last_snapshot) >= SNAPSHOT_INTERVAL)
...
void commit_kv_log(volume_descriptor *volume_desc, db_descriptor * db_desc, int which_dbs){
	...
	qnode_t lock_node;
#endif
	if(which_dbs == (char)ALL_DBS){
		node = getFirst(volume_desc->open_databases);
		while(node != NULL){
			db_desc = (db_descriptor *) (node->data);
			printf("[%s:%s:%d] commiting kv log of db %s\n",__FILE__,__func__,__LINE__,db_desc->db_name);
#ifdef LOG_WITH_MUTEX
			MUTEX_LOCK(&db_desc->lock_log);
#elif SPINLOCK
			pthread_spin_lock(&db_desc->lock_log);
#else
			acquire(&db_desc->lock_log,&lock_node);
#endif

			/*write log info*/
			if(db_desc->g_first_kv_log != NULL)
				db_desc->commit_log->first_kv_log = (block_header *)((uint64_t)db_desc->g_first_kv_log - MAPPED);
			else
				db_desc->commit_log->first_kv_log = NULL;
			if(db_desc->g_last_kv_log != NULL)
				db_desc->commit_log->last_kv_log = (block_header *)((uint64_t)db_desc->g_last_kv_log - MAPPED);
			else
			...
			if(msync(db_desc->commit_log,sizeof(commit_log_info),MS_SYNC) == -1){
				printf("[%s:%s:%d] FATAL msync failed\n",__FILE__,__func__,__LINE__);
				exit(EXIT_FAILURE);
			}
#ifdef LOG_WITH_MUTEX
			MUTEX_UNLOCK(&db_desc->lock_log);
#elif SPINLOCK
			pthread_spin_unlock(&db_desc->lock_log);
#else
			release(&db_desc->lock_log,&lock_node);
#endif
			node = node->next;
		}
		/*hack*/
	}else if(which_dbs == UNIQUE_DB || which_dbs == UNIQUE_DB_ALREADY_LOCKED){
		if(which_dbs == UNIQUE_DB){
#ifdef LOG_WITH_MUTEX
			MUTEX_UNLOCK(&db_desc->lock_log);
#elif SPINLOCK
			pthread_spin_unlock(&db_desc->lock_log);
...
void cow_nodes_related_toroot(db_descriptor* db_desc,node_header * node){
	...
	int32_t i,level_check = 0;
	void * addr;
	node_header * child;
	node_header * cow_child;
	allocator_descriptor temp;
	temp.allocate_space = &allocate_segment;
	temp.handle = NULL;
	temp.level_id = 0;

	if(node->num_entries == 0) return;

	for (i = 0; i < 4; ++i) {
		if(db_desc->root_w[i] == node)
			level_check = 1;
	}

	if(!level_check) temp.level_id = NUM_OF_TREES_PER_LEVEL;
	if(node->type == leafRootNode) return;
	...
	for(i=0;i<node->num_entries;i++){
		...
		cow_child = (node_header *)db_desc->createEmptyNode(&temp, NULL, leafRootNode, NEW_ROOT);
		memcpy(cow_child,child,NODE_SIZE);
		cow_child->epoch += 2;
		*(uint64_t *)addr = (uint64_t)cow_child-MAPPED;
...
void snapshot(volume_descriptor * volume_desc){
		...
		if(db_desc->dirty > 0){
			...
			for(i=0;i<TOTAL_TREES;i++){
				...
				if(db_desc->root_w[i] != NULL){
					...
					if((db_desc->dirty &&i == db_desc->active_tree )|| i == 4){
						if(i != NUM_OF_TREES_PER_LEVEL){
							temp.allocate_space = &allocate_segment;
							temp.handle = single_db;
							temp.level_id = 0;
							/*Level 0*/
							allocate_segment(single_db,BUFFER_SEGMENT_SIZE,i,NEW_LEVEL_0_TREE);
							temp_node = (node_header *)db_desc->createEmptyNode(&temp, single_db, leafRootNode, NEW_ROOT);
							memcpy(temp_node,db_desc->root_r[i],NODE_SIZE);
							temp_node->epoch += 2;
							cow_nodes_related_toroot(db_desc,temp_node);
							db_desc->root_w[i] = temp_node;
						}else{
							/*Level 1*/
							temp.allocate_space = &allocate_segment;
							temp.handle = single_db;
							...
							allocate_segment(single_db,BUFFER_SEGMENT_SIZE,NUM_OF_TREES_PER_LEVEL,NEW_LEVEL_1_TREE);
							temp_node = (node_header *)db_desc->createEmptyNode(&temp, single_db, leafRootNode, NEW_ROOT);
							memcpy(temp_node,db_desc->root_r[NUM_OF_TREES_PER_LEVEL],NODE_SIZE);
							temp_node->epoch += 2;
							cow_nodes_related_toroot(db_desc,temp_node);
							db_desc->root_w[NUM_OF_TREES_PER_LEVEL] = temp_node;
						}
			...
			if(db_desc->db_mode == PRIMARY_DB){
				db_group->db_entries[db_desc->group_index].g_first_kv_log = (block_header *)((uint64_t)db_desc->g_first_kv_log - MAPPED);
				db_group->db_entries[db_desc->group_index].g_last_kv_log =  (block_header *)((uint64_t)db_desc->g_last_kv_log - MAPPED);
				db_group->db_entries[db_desc->group_index].g_kv_log_size = (uint64_t)db_desc->g_kv_log_size;
			} else if(db_desc->db_mode == BACKUP_DB_NO_PENDING_SPILL){
				if(db_desc->g_first_backup_kv_log != NULL)
					db_group->db_entries[db_desc->group_index].g_first_kv_log = (block_header *)((uint64_t)db_desc->g_first_backup_kv_log - MAPPED);
				else
					db_group->db_entries[db_desc->group_index].g_first_kv_log = 0;
				if(db_desc->g_last_backup_kv_log != NULL)
					db_group->db_entries[db_desc->group_index].g_last_kv_log =  (block_header *)((uint64_t)db_desc->g_last_backup_kv_log - MAPPED);
				else
					db_group->db_entries[db_desc->group_index].g_last_kv_log = 0;
				db_group->db_entries[db_desc->group_index].g_kv_log_size = (uint64_t)db_desc->g_backup_kv_log_size;
			}
			else if(db_desc->db_mode == BACKUP_DB_PENDING_SPILL){
				printf("[%s:%s:%d] ommiting persisting log details backup db is in spill mode\n",__FILE__,__func__,__LINE__);
			}
#ifdef SCAN_REORGANIZATION
			db_group->db_entries[db_desc->group_index].leaf_id = db_desc->leaf_id;
#endif
		}
		commit_kv_log(NULL,db_desc,UNIQUE_DB_ALREADY_LOCKED);
		node = node->next;
	}
	...
	/*hack*/
	MUTEX_UNLOCK(&volume_desc->lock_log);
	while (node != NULL){
		db_desc = (db_descriptor *) node->data;
		db_desc->dirty = 0x00;
	...
	__sync_fetch_and_add(&snapshot_v2,1);
	...
	printf("[%s: %s: %d] snapshot finished \n",__FILE__,__func__,__LINE__);
}
