...
void recovery_worker(void *args)
{
	...
	log_info("L0 start offset %llu maps to segment id %llu", (LLU)recover_req->db_desc->big_log_head_offset,
		 (LLU)segment_id);
	current_log_segment = (segment_header *)REAL_ADDRESS(recover_req->db_desc->commit_log->big_log_tail);
	...
	while (log_offset < db_desc->commit_log->big_log_size) {
		if (log_offset == 0) {
		...
		if (log_offset % BUFFER_SEGMENT_SIZE == 0) {
			...
			current_log_segment = (segment_header *)REAL_ADDRESS(current_log_segment->next_segment);
			...
			log_info(
				...
				(LLU)db_desc->commit_log->big_log_size);
		...
		if (KEY_SIZE(kv_addr) > 0) {
			...
			if (remaining_bytes_in_segment <= sizeof(uint32_t)) {
				...
				log_info(
					...
					(LLU)log_offset, (LLU)db_desc->commit_log->big_log_size);
			...
			if (log_offset >= db_desc->commit_log->big_log_size)
				break;
	...
	log_info("finished recovery for db %s log offset = %llu commit log offset = %llu",
		 recover_req->db_desc->db_name, (LLU)log_offset, (LLU)db_desc->commit_log->big_log_size);
	assert(log_offset == (LLU)db_desc->commit_log->big_log_size);
	log_offset = recover_req->db_desc->big_log_size;
