...
#include "core_workload.h"
	...
	#include "UniformLongGenerator.hpp"
	#include "ZipfianGenerator.hpp"
	#include "ScrambledZipfianGenerator.hpp"
	#include "SkewedLatestGenerator.hpp"
	#include "ConstantIntegerGenerator.hpp"
	#define UniformGenerator UniformLongGenerator
	#define ConstGenerator ConstantIntegerGenerator
#else
...
const string CoreWorkload::MAX_EXECUTION_TIME_PROPERTY = "maxexecutiontime";
const string CoreWorkload::MAX_EXECUTION_TIME_DEFAULT = "1000000";

const string CoreWorkload::STATUS_INTERVAL_PROPERTY = "status.interval";
const string CoreWorkload::STATUS_INTERVAL_DEFAULT = "10";
...
/*<gesalous>*/
const string CoreWorkload::DISTRIBUTED_SETUP_PROPERTY = "distributedsetup";
const string CoreWorkload::DISTRIBUTED_SETUP_DEFAULT = "false";

const string CoreWorkload::NUM_OF_CLIENTS_PROPERTY = "numofclients";
const string CoreWorkload::NUM_OF_CLIENTS_DEFAULT = "0";


const string CoreWorkload::PARALLEL_LOADING_PROPERTY = "parallelloading";
const string CoreWorkload::PARALLEL_LOADING_DEFAULT = "false";

const string CoreWorkload::ZOOKEEPER_SERVER_PROPERTY = "zookeeper";
const string CoreWorkload::ZOOKEEPER_SERVER_DEFAULT = "127.0.0.1:2181";
/*</gesalous>*/
...
void CoreWorkload::Init(const utils::Properties &p)
{
  ...
  double readmodifywrite_proportion = std::stod(p.GetProperty(READMODIFYWRITE_PROPORTION_PROPERTY, READMODIFYWRITE_PROPORTION_DEFAULT));
  ...
  if (readmodifywrite_proportion > 0) {
	...
	if(request_dist == "uniform"){
		key_chooser_ = new UniformLongGenerator(insert_start, insert_start + record_count_ - 1);
    ...
    int expectednewkeys = (int)((opcount) * insert_proportion * 2.0); // 2 is fudge factor
    key_chooser_ = new ScrambledZipfianGenerator(insert_start, insert_start + record_count_ + expectednewkeys);
...
#ifdef NEW_GENERATORS
    pair.second.append(field_len_generator_->nextValue(), 'a' /* utils::RandomPrintChar() */);
#else
    pair.second.append(field_len_generator_->Next(), 'a' /* utils::RandomPrintChar() */);
#endif
