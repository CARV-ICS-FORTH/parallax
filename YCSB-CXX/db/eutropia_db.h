...
namespace ycsbc {
  ...
  public:
    EutropiaDB(int num, utils::Properties& props) : db_num(num), field_count(std::stoi(props.GetProperty(CoreWorkload::FIELD_COUNT_PROPERTY, CoreWorkload::FIELD_COUNT_DEFAULT))), dbs()
    {
    ...
    int Update(int id, const std::string &table, const std::string &key, std::vector<KVPair> &values)
    {
      ...
      if(field_count > 1){ // this results in read-modify-write. Maybe we should use merge operator here
        ...
        for(auto f : values){
          ...
          if(it == vmap.end()){
            std::cout << "[2][UPDATE] Cannot find : " << f.first << " in DB " << db_id << std::endl;
            exit(EXIT_FAILURE);
          }
