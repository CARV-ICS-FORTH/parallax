...
int HashtableDB::Read(int id, const string &table, const string &key,
  ...
  if (!field_table) return YCSBDB::kErrorNoData;
...
int HashtableDB::Scan(int id, const string &table, const string &key, int len,
...
int HashtableDB::Update(int id, const string &table, const string &key,
...
int HashtableDB::Insert(int id, const string &table, const string &key,
    vector<KVPair> &values) {
  string key_index(table + key);
  FieldHashtable *field_table = key_table_->Get(key_index.c_str());
  if (!field_table) {
  ...
  for (KVPair &field_pair : values) {
    ...
    if (!ok) {
      DeleteString(value);
      return YCSBDB::kErrorConflict;
    }
  }
  return YCSBDB::kOK;
}

int HashtableDB::Delete(int id, const string &table, const string &key) {
  string key_index(table + key);
