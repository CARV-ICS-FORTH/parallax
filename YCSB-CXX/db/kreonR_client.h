...
public:
	...
	void Close()
	{
		...
		for(i=0;i<MAX_THREADS;i++){
			if(num_of_batch_operations_per_thread[i] > 0){
				cli_tu_region = Client_Get_Tu_Region_and_Mailbox( client_regions, client_buffers[i].buffer+4,*(uint32_t*)client_buffers[i].buffer, 0, &mailbox);
				...
				if(!push_buffer_in_tu_data_message(mr_message, client_buffers[i].buffer,client_buffers[i].pos)){
				...
				++pending_requests[i];
				async_send_rdma_message(connection, mr_message, &callback_function, &served_requests[i]);
	...
	int __read(int id, const std::string &table, const std::string &key, const std::vector<std::string> *fields, std::vector<KVPair> &result)
	{
		...
		cli_tu_region = Client_Get_Tu_Region_and_Mailbox( client_regions, (char *)key.c_str(),key.length(), db_id, &mailbox );
		//struct connection_rdma* connection = get_connection_from_region(cli_tu_region);

		struct connection_rdma* connection = cli_tu_region->head_net->rdma_conn[id % NUM_OF_CONNECTIONS_PER_SERVER];
		std::string server(cli_tu_region->head);
		...
		*(uint32_t *)mr_message->next = key.length();
		...
		if(cnt != field_count){
			std::cout << "ERROR IN VALUE!" << std::endl << " cnt is = " << cnt << "value len " << len_val << "field count is " << field_count << "\n";
	...
	{
		scanner_s* scan_cursor = client_scan_init(const_cast<char*>(key.c_str()), key.length(), NULL, 0, client_regions);
		++pending_requests[id];
    ...
    mr_message = allocate_rdma_message(connection, total_length, type);
		...
		if(type == PUT_REQUEST){
			...
			*(uint32_t *)mr_message->next = pos;
			...
			if(!push_buffer_in_tu_data_message(mr_message, client_buffers[id].buffer,client_buffers[id].pos)) {
				DPRINT("push_buffer for key FAILED\n");
				exit(EXIT_FAILURE);
			}
		}
