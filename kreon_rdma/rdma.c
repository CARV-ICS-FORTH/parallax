...
int assign_job_to_worker(struct channel_rdma *channel, struct connection_rdma *conn, tu_data_message_s *msg, int spinning_thread_id, int sockfd);
uint64_t wake_up_workers_operations = 0;

/*gesalous, helper functions for spinning thread*/
uint32_t  _wait_for_payload_arrival(tu_data_message_s * hdr);
...
void crdma_server_create_connection_inuse(struct connection_rdma* connection, struct channel_rdma *channel,connection_type type );
...
/*gesalous new stuff*/
static int __send_rdma_message(connection_rdma * conn, tu_data_message_s * msg);
static tu_data_message_s * _client_allocate_rdma_message(connection_rdma * conn, int message_payload_size, int message_type);
...
static void _wait_for_reset_buffer_ack(connection_rdma * conn)
{
	...
	tu_data_message_s * reset_buffer_ack = (tu_data_message_s *)((uint64_t)conn->rdma_memory_regions->remote_memory_buffer + payload_end);
...
void _send_reset_buffer_ack(connection_rdma * conn)
{
	...
	msg = (tu_data_message_s *)((uint64_t)conn->rdma_memory_regions->local_memory_buffer + payload_end);
	memset(msg,0, sizeof(tu_data_message_s));
...
void init_rdma_message(connection_rdma * conn, tu_data_message_s * msg, uint32_t message_type, 
		uint32_t message_size,uint32_t message_payload_size, uint32_t padding)
{
	if(message_payload_size > 0){
		...
		*(uint32_t *)(((uint64_t)msg + TU_HEADER_SIZE + msg->pay_len + msg->padding_and_tail) - sizeof(uint32_t)) = TU_RDMA_REGULAR_MSG;
	} else {
		msg->pay_len = 0;
		msg->padding_and_tail = 0;
		msg->data = NULL;
	...
	while(1){
		...
		switch(stat){
			...
			case NOT_ENOUGH_SPACE_AT_THE_END:
				...
				if(allocate_space_from_circular_buffer(conn->send_circular_buf, MESSAGE_SEGMENT_SIZE, &addr) != ALLOCATION_IS_SUCCESSFULL){
					DPRINT("FATAL cannot send reset rendezvous\n");
					exit(EXIT_FAILURE);
				}
				msg  = (tu_data_message_s *)addr;
				msg->pay_len = 0;
				msg->padding_and_tail = 0;
				msg->data = NULL;
				msg->next = NULL;
	...
	if(reply_size > 0){
		while(1){
			...
			switch(stat){
				...
				case BITMAP_RESET:
				case ALLOCATION_IS_SUCCESSFULL:
					msg->reply = (char *)((uint64_t)addr - (uint64_t)conn->recv_circular_buf->memory_region);
...
tu_data_message_s * __allocate_rdma_message(connection_rdma * conn, int message_payload_size, int message_type, int allocation_type, int priority, work_task * task)
{
	tu_data_message_s * msg;
	tu_data_message_s * reset_buffer_ack;
	...
	if(task->allocation_status == ALLOCATION_START && 
	...
	while(1){
		switch(task->allocation_status){
			case ALLOCATION_START:
				...
				local_offset = conn->offset;
				if((local_offset + message_size) > conn->rdma_memory_regions->memory_region_length-MESSAGE_SEGMENT_SIZE){
					//DPRINT("Time to reset buffer\n");
					conn->status = CONNECTION_RESETTING;
					/*reset protocol follows, first wait every pending message to finish*/
					local_offset = conn->offset;
					/*fits exactly one header*/
					msg = (tu_data_message_s *)((uint64_t)conn->rdma_memory_regions->local_memory_buffer + local_offset);
			...
			case CHECK_FOR_RESET_BUFFER_ACK:
				...
				reset_buffer_ack = (tu_data_message_s *)((uint64_t)conn->rdma_memory_regions->remote_memory_buffer + payload_end);
				while(reset_buffer_ack->receive != RESET_BUFFER_ACK){
					++tries;
...
void async_send_rdma_message(connection_rdma *conn, tu_data_message_s *msg, void (*callback_function)(void *args), void * args)
{
...
void client_free_rpc_pair(connection_rdma * conn, tu_data_message_s *msg)
{
	...
	request = (tu_data_message_s *)msg->request_message_local_addr;
...
{
	tu_data_message_s * disconnect_request = allocate_rdma_message(conn, 0, DISCONNECT);
...
int rdma_kv_entry_to_replica(connection_rdma * conn, tu_data_message_s * data_message, 
		uint64_t segment_log_offset, void *source, uint32_t kv_length, uint32_t client_buffer_key)
{
	...
	wr.wr.rdma.remote_addr = (uint64_t)(conn->peer_mr->addr + (uint64_t)data_message->local_offset + TU_HEADER_SIZE + 4096 + segment_log_offset);
...
int wake_up_replica_to_flush_segment(connection_rdma * conn, tu_data_message_s * msg, int wait)
{
	...
	sge_segment_metadata.addr = (uint64_t)((uintptr_t)conn->rdma_memory_regions->local_memory_buffer + msg->local_offset + TU_HEADER_SIZE);
	...
	if(ibv_post_send(conn->qp, &wr_segment_metadata, &bad_wr_segment_metadata)!= 0){
		...
		perror("Reason: ");
		DPRINT("payload length of failed wr is %d and local addr %llu msg len %d\n",
				bad_wr_segment_metadata->sg_list->length, (LLU)bad_wr_segment_metadata->sg_list->addr, msg->pay_len);
		raise(SIGINT);
		exit(EXIT_FAILURE);
	}
...
void*  worker_thread_kernel(void *args)
{
	...
	while(1){
		...
		if(turn == 0){
				/*pending tasks*/
				if(++num_of_processed_requests_per_priority[turn] >= WORKER_THREAD_HIGH_PRIORITY_TASKS_PER_TURN){
				...
				if(++num_of_processed_requests_per_priority[turn] >= WORKER_THREAD_NORMAL_PRIORITY_TASKS_PER_TURN){
					num_of_processed_requests_per_priority[turn] = 0;
					++turn;
				}
		...
		if(job->overall_status == TASK_COMPLETED){
			...
			if(job->msg->type == FLUSH_SEGMENT_AND_RESET || job->msg->type == FLUSH_SEGMENT 
					|| job->msg->type == SPILL_INIT || job->msg->type == SPILL_COMPLETE || job->msg->type == SPILL_BUFFER_REQUEST ){
				assert(utils_queue_push(&worker_descriptor->empty_high_priority_job_buffers_queue,job) != NULL);
			} else {
...
connection_rdma* find_memory_steal_candidate(SIMPLE_CONCURRENT_LIST* connections_list) {
	...
	while (node) {
		if (!((connection_rdma*)node->data)->next_rdma_memory_regions
				&& (ret == NULL || ret->rdma_memory_regions->memory_region_length < ((connection_rdma*)node->data)->rdma_memory_regions->memory_region_length))
	...
	DPRINT("id = %d, ret = %p, ret->mrsize = %zu\n", ret->idconn, ret, ret->rdma_memory_regions->memory_region_length);
...
static void *socket_thread( void *args )
{
	...
	DPRINT("Starting listener for new connections thread at port %d\n",globals_get_RDMA_connection_port());
	...
	while(1){
		...
		if (ret) {
			ERRPRINT("rdma_accept: %s\n", strerror(errno));
			exit(EXIT_FAILURE);
		}

		while (incoming_connection_type == -1) ; // Wait for message to arrive
		rdma_dereg_mr(recv_mr);

		if(incoming_connection_type == CLIENT_TO_SERVER_CONNECTION){
			incoming_connection_type = SERVER_TO_CLIENT_CONNECTION;
			// DPRINT("We have a new client connection request\n");
		}
		...
		crdma_server_create_connection_inuse(conn, channel, incoming_connection_type); // TODO not sure if it's needed with rdma_cm
		conn->rdma_cm_id = new_conn_id;
		switch(conn->type){
			case SERVER_TO_CLIENT_CONNECTION:
				conn->rdma_memory_regions = mrpool_allocate_memory_region(channel->static_pool);
				break;
			case REPLICA_TO_MASTER_CONNECTION:
				conn->rdma_memory_regions = mrpool_allocate_memory_region(channel->dynamic_pool);
				break;
			default:
				ERRPRINT("bad connection type %d\n", conn->type);
				exit(EXIT_FAILURE);
		}
		conn->rdma_memory_regions->local_memory_region = rdma_reg_write(new_conn_id, conn->rdma_memory_regions->local_memory_buffer, conn->rdma_memory_regions->memory_region_length);
		conn->rdma_memory_regions->remote_memory_region = rdma_reg_write(new_conn_id, conn->rdma_memory_regions->remote_memory_buffer, conn->rdma_memory_regions->memory_region_length);
		...
		if(conn->type == SERVER_TO_CLIENT_CONNECTION && !conn->rdma_memory_regions){ /*{{{*/
			...
			// assign_job_to_worker(candidate->channel, candidate, (tu_data_message_s*)577, 0, -1);
		...
		struct ibv_mr* send_mr = rdma_reg_msgs(new_conn_id, conn->rdma_memory_regions->remote_memory_region, sizeof(struct ibv_mr));
		...
		ret = rdma_post_send(new_conn_id, NULL, conn->rdma_memory_regions->remote_memory_region, sizeof(struct ibv_mr), send_mr, 0);
		if (ret) {
			ERRPRINT("rdma_post_send: %s\n", strerror(errno));
			exit(EXIT_FAILURE);
		}
		while (!conn->peer_mr->rkey) ; // Wait for message to arrive
		...
		// DPRINT("\t****** Built new connection successfully  ********\n");
	}
	return NULL;
}
...
void crdma_init_client_connection_list_hosts(connection_rdma *conn, char **hosts, 
		const int num_hosts, struct channel_rdma *channel, connection_type type )
{
	...
	switch(type){
		...
		case CLIENT_TO_SERVER_CONNECTION:
			DPRINT("Remote side accepted created a new CLIENT_TO_SERVER_CONNECTION\n");
			conn->type = CLIENT_TO_SERVER_CONNECTION;
			break;
		default:
			ERRPRINT("BAD connection type\n");
			exit(EXIT_FAILURE);
	}
	switch(conn->type){
		case CLIENT_TO_SERVER_CONNECTION:
	...
	conn->rdma_memory_regions->local_memory_region = rdma_reg_write(conn->rdma_cm_id, conn->rdma_memory_regions->local_memory_buffer, conn->rdma_memory_regions->memory_region_length);
	conn->rdma_memory_regions->remote_memory_region = rdma_reg_write(conn->rdma_cm_id, conn->rdma_memory_regions->remote_memory_buffer, conn->rdma_memory_regions->memory_region_length);

	// Receive server's memory region information
	while (conn->peer_mr->rkey == 0) ; // Wait for message to arrive
	rdma_dereg_mr(send_mr);
	rdma_dereg_mr(recv_mr);
	...
	// Send memory region information
	ret = rdma_post_send(rdma_cm_id, NULL, conn->rdma_memory_regions->remote_memory_region, sizeof(struct ibv_mr), send_mr, 0);
	if (ret) {
		ERRPRINT("rdma_post_send: %s\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if(LIBRARY_MODE == CLIENT_MODE){
		// DPRINT("CLIENT: Initializing client circular buffer *** NEW feature ****\n");
		
		conn->send_circular_buf = create_and_init_circular_buffer(conn->rdma_memory_regions->local_memory_buffer, conn->peer_mr->length,MESSAGE_SEGMENT_SIZE, SEND_BUFFER);
		conn->recv_circular_buf = create_and_init_circular_buffer(conn->rdma_memory_regions->remote_memory_buffer, conn->peer_mr->length,MESSAGE_SEGMENT_SIZE, RECEIVE_BUFFER);
		conn->reset_point = 0;
		...
		/*control info*/
		msg->reply = (char*) (conn->recv_circular_buf->bitmap_size *BITS_PER_BITMAP_WORD*MESSAGE_SEGMENT_SIZE);
	...
	crdma_add_connection_channel( channel, conn );
	// DPRINT("Client Connection build successfully\n");
...
void crdma_init_generic_create_channel(struct channel_rdma * channel)
{
	...
	for( i = 0; i < channel->spinning_num_th; i ++ ){
		...
		spinning_thread_parameters * params = (spinning_thread_parameters *)malloc(sizeof(spinning_thread_parameters));
		params->channel = channel;
		params->spinning_thread_id = i;


		if(LIBRARY_MODE == SERVER_MODE){
			if(pthread_create( &channel->spinning_thread[i], NULL, server_spinning_thread_kernel, params ) != 0){
				free(channel);
			...
			if(pthread_create( &channel->spinning_thread[i], NULL, client_spinning_thread_kernel, params ) != 0){
				free(channel);
		...
		if(LIBRARY_MODE == SERVER_MODE){
			...
			status = pthread_setaffinity_np(channel->spinning_thread[i], sizeof(cpu_set_t), &spinning_thread_affinity_mask);
			if(status != 0){
			...
			channel->spinning_thread_group[i] = (worker_group *)malloc(sizeof(worker_group) + (WORKER_THREADS_PER_SPINNING_THREAD * sizeof(worker_thread)));
			channel->spinning_thread_group[i]->group = (worker_thread *)((uint64_t)channel->spinning_thread_group[i] + sizeof(worker_group));
			channel->spinning_thread_group[i]->next_server_worker_to_submit_job = 0;
			channel->spinning_thread_group[i]->next_client_worker_to_submit_job = WORKER_THREADS_PER_SPINNING_THREAD/2;
			...
			for(j=0; j<WORKER_THREADS_PER_SPINNING_THREAD; j++){
				...
				utils_queue_init(&channel->spinning_thread_group[i]->group[j].empty_high_priority_job_buffers_queue);
				...
				for(k=0;k<UTILS_QUEUE_CAPACITY;k++){
					utils_queue_push(&channel->spinning_thread_group[i]->group[j].empty_job_buffers_queue,
							&channel->spinning_thread_group[i]->group[j].job_buffers[k]);

					utils_queue_push(&channel->spinning_thread_group[i]->group[j].empty_high_priority_job_buffers_queue,
							&channel->spinning_thread_group[i]->group[j].high_priority_job_buffers[k]);
				}
				pthread_spin_init(&channel->spinning_thread_group[i]->group[j].work_queue_lock, PTHREAD_PROCESS_PRIVATE);
				channel->spinning_thread_group[i]->group[j].worker_id = j;
				channel->spinning_thread_group[i]->group[j].my_group = channel->spinning_thread_group[i];
			...
			for(j=0; j<WORKER_THREADS_PER_SPINNING_THREAD; j++){
				...
				status = pthread_setaffinity_np(channel->spinning_thread_group[i]->group[j].thread, sizeof(cpu_set_t), &worker_threads_affinity_mask);
				if(status != 0){
		...
		if(LIBRARY_MODE == SERVER_MODE){
			status = pthread_setaffinity_np(channel->cq_poller_thread, sizeof(cpu_set_t), &worker_threads_affinity_mask);
			if(status != 0){
				DPRINT("FATAL failed to pin spinning thread\n");
				exit(EXIT_FAILURE);
			}
...
connection_rdma *crdma_client_create_connection_list_hosts( struct channel_rdma *channel, char **hosts, int num_hosts, connection_type type )
{
	connection_rdma *conn;
	/*allocate memory for connection*/
	conn = malloc( sizeof(connection_rdma));
	if(conn == NULL){
		DPRINT("FATAL ERROR malloc failed\n");
		exit(EXIT_FAILURE);
	}
...
void crdma_server_create_connection_inuse( struct connection_rdma* conn, struct channel_rdma *channel, connection_type type )
{
...
int assign_job_to_worker(struct channel_rdma *channel, struct connection_rdma *conn, tu_data_message_s * msg, int spinning_thread_id, int sockfd)
{
	...
	if(conn->worker_id == -1){
		...
		if(++channel->spinning_thread_group[spinning_thread_id]->next_server_worker_to_submit_job >= WORKER_THREADS_PER_SPINNING_THREAD){
	...
	if(msg->type == FLUSH_SEGMENT_AND_RESET || msg->type == FLUSH_SEGMENT 
			|| msg->type == SPILL_INIT || msg->type == SPILL_COMPLETE || msg->type == SPILL_BUFFER_REQUEST ){
		/*to ensure FIFO processing , vital for these protocol operations*/

		worker_id = conn->worker_id;
		job = (work_task *)utils_queue_pop(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].empty_high_priority_job_buffers_queue);
		if(job == NULL){
			//assert(0);
		...
		job = (work_task *)utils_queue_pop(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].empty_job_buffers_queue);
		if(job == NULL){
			//assert(0);
			return KREON_FAILURE;
		}
	...
	if(msg->type == FLUSH_SEGMENT_AND_RESET || msg->type == FLUSH_SEGMENT 
		...
		if(utils_queue_push(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].high_priority_queue,(void *)job) == NULL){

			DPRINT("\tFailed to add CONTROL job of type: %d to queue tried worker %d its status is %llu retrying\n",
					job->msg->type,worker_id, (LLU)channel->spinning_thread_group[spinning_thread_id]->group[worker_id].status);
			utils_queue_push(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].empty_high_priority_job_buffers_queue,(void *)job);
			assert(0);
			return KREON_FAILURE;
		}
	} 
		...
		/*normal priority*/
		if(utils_queue_push(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].work_queue,(void *)job) == NULL){

			DPRINT("\tFailed to add job of type: %d to queue tried worker %d its status is %llu retrying\n",
					job->msg->type,worker_id, (LLU)channel->spinning_thread_group[spinning_thread_id]->group[worker_id].status);
			utils_queue_push(&channel->spinning_thread_group[spinning_thread_id]->group[worker_id].empty_job_buffers_queue,(void *)job);
			//assert(0);
			return KREON_FAILURE;
		}
	}
...
void _zero_rendezvous_locations(tu_data_message_s *msg)
{
	...
	while(start_memory < end_memory){
		((tu_data_message_s *)start_memory)->receive = 0;
		//((tu_data_message_s *)start_memory)->reply = (void *)0xF40F2;
		//((tu_data_message_s *)start_memory)->reply_length = 0;
...
uint32_t  _wait_for_payload_arrival(tu_data_message_s * hdr)
{
	...
	if(hdr->pay_len > 0){
		...
		tail = (uint32_t *)(((uint64_t)hdr + TU_HEADER_SIZE + hdr->pay_len + hdr->padding_and_tail) - sizeof(uint32_t));
...
static void __stop_client(connection_rdma * conn)
{
	...
	tu_data_message_s *msg = (tu_data_message_s *)((uint64_t)conn->rdma_memory_regions->local_memory_buffer + (uint64_t)conn->control_location);
...
void _update_client_rendezvous_location(connection_rdma * conn, int message_size)
{
		...
		assert(message_size % MESSAGE_SEGMENT_SIZE == 0);
		if((uint64_t)conn->rendezvous + message_size >= (uint64_t)conn->rdma_memory_regions->remote_memory_buffer + (conn->peer_mr->length - MESSAGE_SEGMENT_SIZE)){
			conn->rendezvous = conn->rdma_memory_regions->remote_memory_buffer;
		} else {
...
void _update_rendezvous_location(connection_rdma * conn, int message_size)
{
	...
	if(conn->type == SERVER_TO_CLIENT_CONNECTION){
		...
		if(((uint64_t)conn->rendezvous + message_size) >= ((uint64_t)conn->rdma_memory_regions->remote_memory_buffer + conn->rdma_memory_regions->memory_region_length)){
	...
	while(1){
		...
		while(node != NULL){
			...
			if(conn->status == STOPPED_BY_THE_SERVER){
				...
				if((conn->recv_circular_buf->bitmap[conn->recv_circular_buf->bitmap_size-1])!= 0x7FFFFFFF){
...
check_done:
				if(all_requests_completed == 1){
					...
					tu_data_message_s * reset = _client_allocate_rdma_message(conn,0,SERVER_I_AM_READY);
					...
					int bitmap_size = conn->next_peer_mr->length/MESSAGE_SEGMENT_SIZE/BITS_PER_BITMAP_WORD;
					reset->data = bitmap_size *BITS_PER_BITMAP_WORD*MESSAGE_SEGMENT_SIZE - MESSAGE_SEGMENT_SIZE;
				...
				hdr = (tu_data_message_s*)((uint64_t)conn->rdma_memory_regions->remote_memory_buffer + (uint64_t)conn->control_location);
				recv = hdr->receive;
				int stat;
				int spin_counter = 0;
					...
					switch(hdr->type){
						case CLIENT_STOP_NOW:
							...
							if(stat == 0){
								...
								DPRINT("old_mr: addr = %p. length = %lu, rkey = %u\n", conn->peer_mr->addr, conn->peer_mr->length, conn->peer_mr->rkey);
								conn->next_peer_mr = malloc(sizeof(struct ibv_mr));
								memcpy(conn->next_peer_mr, (char*)hdr + sizeof(tu_data_message_s), sizeof(struct ibv_mr));
								DPRINT("new_mr: addr = %p. length = %lu, rkey = %u\n", conn->next_peer_mr->addr, conn->next_peer_mr->length, conn->next_peer_mr->rkey);
			...
			hdr = (tu_data_message_s*)conn->rendezvous;
			recv = hdr->receive;
			...
			if(recv == TU_RDMA_REGULAR_MSG ){
				...
				if(hdr->request_message_local_addr !=  NULL){
					...
					tu_data_message_s * request = (tu_data_message_s *)hdr->request_message_local_addr;
					...
					if((request->flags & 0x000000FF) == SYNC_REQUEST){
						...
						DPRINT("\t FATAL unknown flags request addr: %llu of type %d reply is %d flags %d recv is %d\n",
								(LLU)request, request->type, hdr->type, request->flags, hdr->receive);
						raise(SIGINT);
						exit(EXIT_FAILURE);
					}
				} else {
			...
			if(node->marked_for_deletion){
				...
				delete_element_from_simple_concurrent_list(channel->spin_list[spinning_thread_id], prev_node, node);
				node = next_node;
	...
	while(1){
		...
		while(node != NULL){
			...
			if(recv == TU_RDMA_REGULAR_MSG ){
				...
				if(rc != KREON_FAILURE){
					hdr->receive = 0;/*responsible worker will zero and update rendevous locations*/
				}
				goto iterate_next_element;
				...
				if(hdr->type == DISCONNECT){
					...
					if (req->desired_priority_level == HIGH_PRIORITY) {
						...
						tu_data_message_s * reply = allocate_rdma_message(conn, 0, CHANGE_CONNECTION_PROPERTIES_REPLY);
					...
					assert(0);
					((tu_data_message_s *)hdr->request_message_local_addr)->ack_arrived = REPLY_ARRIVED;
					/*do nothing for now*/
					_zero_rendezvous_locations(hdr);
					_update_rendezvous_location(conn, message_size);
					goto iterate_next_element;
...
iterate_next_element:
			if(node->marked_for_deletion){
				...
				if(spinning_list_type == HIGH_PRIORITY)
					...
					delete_element_from_simple_concurrent_list(channel->idle_conn_list[spinning_thread_id], prev_node, node);
				node = next_node;
				pthread_mutex_unlock( &channel->spin_list_conn_lock[spinning_thread_id] );
			}
				...
				pthread_mutex_lock( &channel->spin_list_conn_lock[spinning_thread_id] );
				next_node = node->next;/*Caution prev_node remains intact*/
				remove_element_from_simple_concurrent_list(channel->spin_list[spinning_thread_id], prev_node, node);
			...
			else if(spinning_list_type == LOW_PRIORITY && 
					conn->idle_iterations > MAX_IDLE_ITERATIONS){
...
upgrade_connection:
				...
				pthread_mutex_lock( &channel->spin_list_conn_lock[spinning_thread_id] );
				next_node = node->next;/*Caution prev_node remains intact*/
				remove_element_from_simple_concurrent_list(channel->idle_conn_list[spinning_thread_id], prev_node, node);
	...
	if (wc->status == IBV_WC_SUCCESS){
		switch(wc->opcode){
			...
			case IBV_WC_RDMA_WRITE:
				if(wc->wr_id != 0){
					msg  = (tu_data_message_s *)wc->wr_id;
					switch(msg->type){
