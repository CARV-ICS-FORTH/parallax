...
#define KEY_PRINT_FMT_GID "%04x:%06x:%06x:%08x:%016Lx:%016Lx:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:"
...
#define CONNECTION_PROPERTIES 9/*not a message type used for recv flags in messages to indicate that either a
...
#define TU_RDMA_ACK_RECEIVED_MREND_REPLY_MSG 12  //To inform the client we received the MREND_REPLY_MSG and can be released

#define TU_RDMA_RECEIVED_ACK_MSG 14 //To inform the last message we have received. It should be usually sent from the client to the server.
...
typedef enum region_status
{
	REGION_OK = 1000,
	REGION_IN_TRANSITION
}region_status;
...
struct channel_rdma {
	...
	worker_group * spinning_thread_group [SPINNING_NUM_TH]; /*gesalous new staff, references to worker threads per spinning thread*/
...
typedef struct connection_rdma
{
	...
	/*</gesalous>*/
#if TU_RDMA_MEMORY_REGIONS
	//struct mem_reg_q *local_mrq;            // Memory region for RDMA operations. For sending
	//struct remote_mem_reg_q *remote_mrq;    // Memory region for RDMA operations. For receiving
#endif
	// void *rdma_local_region;
	// void *rdma_remote_region;
	/*gesalous staff*/
	// size_t rdma_local_region_length;
	// size_t rdma_remote_region_length;
	...
	// Used to send ACKs back to the remote peer
...
void crdma_init_client_connection(struct connection_rdma *conn, const char *host, const char *port, struct channel_rdma *channel );
...
struct connection_rdma *crdma_client_create_connection_list_hosts(struct channel_rdma *channel, 
		char **hosts, 
		int num_hosts, 
		connection_type type);

void crdma_init_client_connection_list_hosts(struct connection_rdma *conn, 
		char **hosts, 
		const int num_hosts, 
		struct channel_rdma *channel, 
		connection_type type );

int crdma_send_rdma_tucana_message( struct connection_rdma *conn, uint32_t length,  struct tu_data_message *data_message, int re_send );
...
tu_data_message_s * __allocate_rdma_message(connection_rdma * conn, int message_payload_size, int message_type, int rdma_allocation_type, int priority, work_task * task);
...
int rdma_kv_entry_to_replica(connection_rdma * conn, tu_data_message_s * data_message, uint64_t segment_log_offset, void *source,
